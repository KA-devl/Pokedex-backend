  ...
  app.post("/api/signup", (req, res) => {
    User.findOne({ where: { username: req.body.username } })
      .then((user) => {
        ...
        if (reg.test(req.body.password) == false) {
        ...
        bcrypt
          .hash(req.body.password, 10) //if everything is correct, hash the password and push it safely to the db
          .then((hash) =>
          ...
          .catch((error) => {
            if (error instanceof ValidationError) {
              ...
              const message =
                "Sorry, we are unable to create a new account at the moment.";
